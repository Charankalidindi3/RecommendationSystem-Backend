...
from .serializers import ItemSerializer,UserSerializer,UserActivitySerializer
...
class UserActivitySet(viewsets.ViewSet):
        lookup_field = 'id'  # optional, defaults to pk
        permission_classes=[IsAuthenticated]
        ...
        def retrieve(self, request, id=None):   # <-- GET /items/{id}/
            item = get_object_or_404(Products, id=id)
            serializer = UserActivitySerializer(item)
            return Response(serializer.data)

        def update(self, request, id=None):     # <-- PUT /items/{id}/
            item = get_object_or_404(Products, id=id)
            serializer = UserActivitySerializer(item, data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
